...
> All 176 PRs are draft. Merge in order. One human click per step.
...
| 119 | `feat/use-cases-external-link-fix` | homepage USE_CASES map: external href (zoostr.xyz) was wrapped in &lt;Link&gt; — fix to native &lt;a target="_blank" rel="noopener noreferrer"&gt;; arrow updated → ↗ |
...
| 129 | `feat/split-wizard-fee-allocation-key` | SplitWizard.tsx: rename exported JSON key feeDistribution → feeAllocation |
| 130 | `feat/ship-md-pr-count-129` | SHIP.md: count 128→130; add rows for PRs #129–#130 |
| 131 | `feat/homepage-fee-claim-language` | page.tsx: community leaderboard description 'pays the people' → 'allocates fee share to the people … claimed at splits.org' |
...
| 173 | `feat/rewards-one-time-price-fix` | rewards page: remove /mo subscription labels — checkout is mode:payment (one-time); subscriptions deferred per V1-SCOPE |
| 174 | `feat/ship-md-pr-count-173` | SHIP.md: count 172→174; add rows for PRs #173–#174 |
| 175 | `feat/discover-ipfs-attested-fix` | discover page: remove false "IPFS-attested" claim from collab EP building example — wizard exports JSON; no IPFS upload in V1 |
