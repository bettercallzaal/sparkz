...
1. **Thin Meme Engine, not dumb.** Keep the v1 loop (human flags moment -> Sparkz drafts 3 responses -> human approves -> publish -> community remixes -> contributors rewarded), but build every component as a replaceable adapter. `interface SignalSource { detectSignals(capsuleId): Promise<CulturalSignal[]> }`. Sources grow from human submission to Farcaster activity to Alpha Radar to predictive intelligence - no rebuild.
   ...
   > **Shipped (2026-08):** the four adapter seams (signal-source, backing-provider, approval-channel, media-kit) are now unified behind a **Capsule plugin system** modeled on elizaOS (`src/lib/plugins`). A `CapsulePlugin` bundles components; `registerPlugin()` fans them into the seams; the `capsule_plugins` table (migration 0004) holds per-Capsule enable/disable + config so each Spark connects to its own communities. Draft generation also became memory-grounded - it now trains on the Capsule's approved winners + passed-on drafts (`meme-engine/draft.ts`), and the moat feedback loop closes. Two capabilities already ship AS plugins: a Farcaster signal source (`/api/signals/detect`) and a Telegram approval channel.
