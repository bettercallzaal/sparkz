# Sparkz - Build Milestone 1 (the Hearth foundation)
...
1. **Hearth schema (Supabase).** One `hearths` table supporting all 4 types
   (`type: creator | culture | oss | meme`), v1 seeds a Creator Hearth for Zoostr.
   Plus: `hearth_backers` (pre-token "spark" backing - no token required),
   `meme_receipts` (THE data moat - see fields below), `signals` (flagged cultural
   moments). Design it so a Culture Hearth or an OSS-repo Hearth is just a
   different `type` + config, not a new schema.
2. **`SignalSource` adapter interface** (the future-proofing that avoids a rebuild):
   `interface SignalSource { detectSignals(hearthId: string): Promise<CulturalSignal[]> }`
...
3. **The human-in-the-loop Meme Engine core loop** (no autonomy in v1):
   flag a signal -> Sparkz drafts **3 Hearth-grounded responses** (use the cheap
...
4. **Meme Receipt fields** (write one per drafted/approved response, cheap now,
   irreplaceable later): hearth_id, original_signal, why_it_matched_the_hearth,
...
5. **A minimal UI**: create/seed the Zoostr Hearth, flag a signal, see the 3 drafts,
...
testing. Each audited project is a **Hearth candidate** - the audit is the
Spark-readiness pass. So the `hearths` schema's OSS-repo type should be able to
import an existing ZAO repo as a Hearth (repo + contributors + audit result +
receipts). CoCConcertZ in particular is slated to become a Spark. Build the schema
with that in mind: a Hearth can wrap a repo, not just a creator.

## First moves for the build terminal

1. Brainstorm the Hearth + Meme Receipt schema with Zaal (one round, then confirm).
