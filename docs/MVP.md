...
A brand can (1) have a Hearth page, (2) get real people to **back/boost** it,
(3) run the **Meme Engine** to produce + publish real Hearth-grounded content,
...
|----------------|--------|------|
| 1. Hearth page (identity, live) | DONE | `/c/[slug]`, 7 brands live |
...
| 5. Receipts + backing accumulate | DONE | the moat, visible on the Hearth page |
...
- **`OPENROUTER_API_KEY` in Vercel** -> real Hearth-grounded drafts (it's set locally,
...
1. **Hearth exists** - it's imported (oss) or created (`/admin/new`). 7 ZAO brands
   are already in.
...
3. **Run the loop** - `/admin` -> pick the Hearth -> flag a real cultural moment ->
...
5. **Point the community at the Hearth page** - `/c/[slug]` - let them **boost**,
