...
| [Admin-less graduation](admin-less-graduation.md) | **BUILD** (#190): B20 makes "nobody controls this token" a provable on-chain state (admin-less launch). Sparkz defaults graduations to admin-less + discloses the role config - a differentiator nobody advertises. Interface-partner 40% split flagged REPORTED, not shipped. |
| [Farcaster Mini Apps](farcaster-mini-apps.md) | **BUILD** (#189): Sparkz was a Mini App that never called `addMiniApp()` - unaddable, so no retention and no notification eligibility. Shipped the add-to-app prompt + `canonicalDomain`. Notifications tracked as the follow-on. |
| [Onboarding friction / create flow](onboarding-friction-create-flow.md) | **BUILD**: sub-20s registration (Linear/Vercel), value action within 5 min (Figma loads a sample), play-first/wallet-later cuts friction up to 90% - which validates the spark model. Shipped four zero-dep create-flow wins: auto-focus, Enter-to-submit, a "spark lit" celebration on redirect, and a live-example empty state. |
