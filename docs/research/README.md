...
| [Farcaster Mini Apps](farcaster-mini-apps.md) | **BUILD** (#187): Sparkz was a Mini App that never called `addMiniApp()` - unaddable, so no retention and no notification eligibility. Shipped the add-to-app prompt + `canonicalDomain`. Notifications tracked as the follow-on. |
