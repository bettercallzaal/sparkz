...
import { importEmpireAsCapsule } from "@/lib/empire/import";
...
// pulls its identity from Empire Builder and creates a Capsule wired to it (idempotent).
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      const capsule = await importEmpireAsCapsule(parsed.data.empire_id.trim());
    ...
    return serverError(err, "capsules.importEmpire.POST");
  }
}
