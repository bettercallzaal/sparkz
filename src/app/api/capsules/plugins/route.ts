...
// GET  /api/capsules/plugins?capsule_id=... - the Hearth's full plugin state
...
export async function GET(req: NextRequest) {
  try {
    ...
    return ok({ capsule_id: hearthId, plugins: state });
  } catch (err) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    await setHearthPlugin(parsed.data.capsule_id, parsed.data.plugin_id, {
      enabled: parsed.data.enabled,
      config: parsed.data.config,
    });

    const state = await getHearthPluginState(parsed.data.capsule_id);
